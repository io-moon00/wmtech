...
class ContactForm(forms.Form):
    ...
    phone = forms.CharField(max_length=20, required=False,  widget=forms.TextInput(attrs={'class': 'block p-3 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 shadow-sm focus:ring-main focus:border-main'}))
    ...
    captcha = ReCaptchaField(widget=ReCaptchaV3(), label=False, error_messages={
        'inv': 'reCaptcha-Validierung fehlgeschlagen. Bitte versuche Sie s später noch einmal oder kontaktieren Sie mich via anfrage@wenger-metalltechnik.ch, wenn das Problem weiterhin besteht.'
    })